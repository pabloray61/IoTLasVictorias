	...
	if (is_uploaded_file($HTTP_POST_FILES["archivo"]['tmp_name'])) {
		if($HTTP_POST_FILES["archivo"]['size'] < 1000000) {
			if($HTTP_POST_FILES["archivo"]['type']=="image/gif" || $HTTP_POST_FILES["archivo"]['type']=="image/jpeg" || $HTTP_POST_FILES["archivo"]['type']=="image/pjpeg") {
				...
				$subio = true;
				$msg = "<img src='images/b_ok.png' border='0'>&nbsp;<font class='send_ok'>La foto (". $archivo .") se agrego correctamente</font>";
			}else{
				$msg = "<img src='images/b_no.png' border='0'>&nbsp;<font class='send_no'>La foto (". $archivo .") no se cargo. El archivo no es del formato GIF o JPG</font>";
			}
		}else{
			$msg = "<img src='images/b_no.png' border='0'>&nbsp;<font class='send_no'>La foto (". $archivo .") no se cargo. El archivo pesa mas de 300 KB</font>";
		}
	}
	unset($agregar);
}
...
function confirmarA(){
		return confirm("Estás seguro que queres agregar esta foto?");
}
			...
			<? if(!$_REQUEST['agregar']){?>
				<table align="center" cellpadding="0" cellspacing="0" border="0" width="450">
					<form action="<?=$PHP_SELF?>" method="POST" name="formu" enctype="multipart/form-data" onSubmit="return confirmarA();">
						...
						<tr height="50"><td class="t" colspan="4" align="center">Subir fotos de <?= $seccion?></td></tr>
						<tr height="60">
							<td width="5%" class="c2">&nbsp;</td>
							<td width="30%" class="c2"><div align="center">Foto</div></td>
			...
			<? }else{?>
				<form action="<?=$PHP_SELF?>" method="POST" name="formulario">
					<table align="center" cellpadding="0" cellspacing="0" border="0" width="450" height="150">
						<tr height="40"><td class="t" align="center">Subir fotos de <?= $seccion?></td></tr>

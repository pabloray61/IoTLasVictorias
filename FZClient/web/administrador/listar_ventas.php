...
if($_REQUEST['eliminar'] == "ok"){ // -------------------- BORRAR ----------------------------

	$qp = query("SELECT * FROM pagos WHERE id_venta=". $_REQUEST['id_venta']);
	if(mysql_num_rows($qp) == 0){
		$qb = query("SELECT id_deposito, cant, id_producto FROM ventas_detalle vd 
					INNER JOIN ventas v ON vd.id_venta = v.id_venta 
					WHERE v.id_venta=". $_REQUEST['id_venta']);
		// MODIFICO EL STOCK
		$i = 0;
		while($rb = mysql_fetch_array($qb)){
			...
			$cant = $rb['cant'];
			if($cant != 0){	
				$q = query("SELECT * FROM stock WHERE id_producto = ". $rb['id_producto'] . " && id_deposito = ". $rb['id_deposito']);
				$rs = mysql_fetch_array($q);
				$c = $rs['cantidad'];
				if($c == "") $c = 0;
				if($imprimir == "ok") echo("Q: ".$cant." - QD: ".$c."<br>");
				if($c > 0){		
					$c_s = $c + $cant;
					$sqls = "UPDATE stock SET cantidad = $c_s where id_producto = ". $rb['id_producto'] . " && id_deposito = ".$rb['id_deposito'];
					if($insertar == "ok") mysql_query($sqls, $conn_id);
					if($imprimir == "ok") echo($sqls."<br>");
				}
				else{
					$sqls = "INSERT INTO stock VALUES (NULL, ".$rb["id_deposito"].",".$rb['id_producto'].", ".$cant.")";
					if($insertar == "ok") mysql_query($sqls, $conn_id);
					if($imprimir == "ok") echo($sqls."<br>");
				}
			}
		...
		$sql="DELETE FROM ventas WHERE id_venta=". $_REQUEST['id_venta'];
		if($imprimir == "ok") echo $sql . "<br>";
		if($insertar == "ok") query($sql);
		
		$msg = "<img src='images/b_ok.png' border='0'>&nbsp;<font class='send_ok'>El venta se elimino correctamente</font>";	
	}else{
		$msg = "<img src='images/b_no.png' border='0'>&nbsp;<font class='send_no'>La venta no se pudo borrar debido a que ya hay un pago asociado a ella.</font>";
	}
}
if($_REQUEST['agregar'] == "agregar_numero"){
	$id_venta = $_REQUEST['id_venta'];
	$ajuste = $_REQUEST['ajuste'];
	$nr = $_REQUEST['n_remito'];
	$fr = rotarFecha($_REQUEST['f_remito']);
	$nf = $_REQUEST['n_factura'];
	$ff = rotarFecha($_REQUEST['f_factura']);
	$sqls = "UPDATE ventas SET ajuste=$ajuste, n_remito='$nr', f_remito='$fr', n_factura='$nf', f_factura='$ff' WHERE id_venta = $id_venta";
	if($insertar == "ok") mysql_query($sqls, $conn_id);
	if($imprimir == "ok") echo($sqls."<br>");
}
if(!empty($_REQUEST['inc'])){
	$id_venta = $_REQUEST['id_venta'];
	$incobrable = $_REQUEST['incobrable'];
	$sqli = "UPDATE ventas SET incobrable=$incobrable WHERE id_venta = $id_venta";
	if($insertar == "ok") mysql_query($sqli, $conn_id);
	if($imprimir == "ok") echo($sqli."<br>");
}
?>
...
<style>
.b_incobrable{ background: url(images/b_incobrable.png) no-repeat 0 0; background-position: 0 -32px; width: 32px; height: 32px; display:inline-block;}
.b_incobrable:hover { background-position: 0 0; }

.b_incobrable_ac{ background: url(images/b_incobrable.png) no-repeat 0 0; width: 32px; height: 32px; display:inline-block;}
</style>
...
?>
	<form name="formu" action="<?= $_SERVER['PHP_SELF']."?".ahora()?>" method="post">
        <table width="600" align="center" cellpadding="0" cellspacing="0" border="0">
        	<tr height="40"><td colspan="3" class="t" align="center">Modificar la Venta</td></tr>
            <tr height="40">
            	<td class="c1" align="center">Ajuste</td>
                <td class="c1" align="left" colspan="2"><input type="text" name="ajuste" value="<?= $ajuste?>" size="5" class="input_grande" onkeyup="cambiar(event);"></td>
            </tr>
            <tr height="40">
	            <td class="c2" align="center">Fecha de Remito</td>
                <td class="c2" align="left"><? agregarFecha("f_remito", $f_remito)?></td>
                <td class="c2" align="left">&nbsp;&nbsp;Numero de Remito&nbsp;&nbsp;<input type="text" name="n_remito" value="<?= $n_remito?>" size="20" class="input_grande"></td>
            </tr>
            <tr height="40">
            	<td class="c1" align="center">Fecha de Factura</td>
                <td class="c1" align="left"><? agregarFecha("f_factura", $f_factura)?></td>
                <td class="c1" align="left">&nbsp;&nbsp;Numero de Factura&nbsp;&nbsp;<input type="text" name="n_factura" value="<?= $n_factura?>" size="20" class="input_grande"></td>
            </tr>
            <tr height="60">
                <td align="center" colspan="3" class="t">
...
<? }else{?>
    ...
    <table width="98%" align="center" cellpadding="0" cellspacing="0" border="0">
        ...
        <tr height="30"><td class="t">&nbsp;</td><td class="t" align="left">Listado de ventas</td><td class="t">&nbsp;</td></tr>
    ...
    if($buscar == "4"){
        ...
        if(!empty($fecha_desde) AND !empty($fecha_hasta)){
            $where = "WHERE fecha BETWEEN '$fecha_desde' AND '$fecha_hasta'";
        }else{
            $where = "WHERE fecha = ".date("Y-m-d");
        }
    }
    
    $sql = "SELECT v.id_venta, v.fecha, v.factura, c.id_cliente, c.nombre, c.apellido, sigla, s.nombre AS vendedor, 
            v.id_vendedor, dto_porc, dto_fijo, com_porc, com_fijo, n_remito, f_remito, n_factura, f_factura, 
			incobrable, ajuste
			FROM ventas v 
                ...
                <div id="container">
                    <div class="demo_jui">    
                        <table width="100%" cellpadding="0" cellspacing="0" border="0" class="display" id="example">
                            <thead>
                                <tr>
                                    <th class="t" width="8%" align="center">Fecha</th>
                                    <? 	if($_SESSION['admin']=="ok"){?>
                                        <th class="t" width="20%">Cliente</th>
                                        <th class="t" width="6%" align="center">V</th>
                                    <? }else{?>
                                        <th class="t" width="26%" colspan="2">Cliente</th>
                                    <? }?>
                                    <th class="t" width="8%" align="center">Monto</th>
                                    <th class="t" width="8%" align="center">Com</th>
                                    <th class="t" width="7%" align="center">Ajuste</th>
                                    <th class="t" width="8%" align="center">Remito</th>
                                    <th class="t" width="8%" align="center">Factura</th>
                                    <th class="t" width="7%" align="center" title="VENTA INCOBRABLE">INC</th>
                            ...
                            <tbody>
                                ...
                                while($rs=mysql_fetch_array($q)){
                                    ...
                                    <tr height="40">
                                        ...
                                        <td align="center">
                                            <a href="<?= $_SERVER['PHP_SELF']?>?acc=ajuste&id_venta=<?=$rs['id_venta']?>" class="link_ventas">
                                            <? if(empty($ajuste)){?>
                                                <img src="images/add.png" border="0" title="Agregar un ajuste">
                                            <? }else{
                                                echo($ajuste);
                                            }?>
                                            </a>
                                        ...
                                        <td class="<?= $c?>" align="center"><a href="<?= $_SERVER['PHP_SELF']?>?id_venta=<?=$rs['id_venta']?>&inc=ok&incobrable=<?= $incobrable == 0 ? 1 : 0;?>" class="b_incobrable<?= $incobrable == 1 ? "_ac":""?>" title="<?= $incobrable == 1 ? "Restablecer la venta" : "Declarar la venta INCOBRABLE"?>"></a></td>

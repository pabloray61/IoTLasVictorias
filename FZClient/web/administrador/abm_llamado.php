...
if ($_REQUEST['agregar'] == "ok"){ // ------------------------------- AGREGAR ---------------------------
	...
	$j = 0;
	$id_vendedor = $_SESSION["id_vendedor"];
	$sql1 = "INSERT INTO llamados VALUES (NULL, $id_vendedor, $id_cliente, '$fecha', '$observaciones')";
	...
	$a = $rs_cl["apellido"];
	if($n != "") $a = $a . ", " . $n;
	$mss_ok = "El llamado al cliente $a se genero correctamente.";
...
if ($_REQUEST['modificar'] == "ok"){ // ------------------------------- MODIFICAR ---------------------------
	...
	$id_vendedor = $_SESSION["id_vendedor"];
	$sql1 = "UPDATE llamados SET fecha= '$fecha', observaciones= '$observaciones' WHERE id_llamado= ".$_REQUEST['id_llamado'];
	if($insertar == "ok") query($sql1);
	if($imprimir == "ok") echo $sql1 . "<br>";
...
?>
	<table width="800" align="center" cellpadding="0" cellspacing="1" border="0">
		...
		<tr height="30"><td class="t" colspan="4" align="center">Agregar un LLamado</td></tr>
			...
			<form action="<? $_SERVER['PHP_SELF']?>" name="formD" method="post">
				...
				<tr height="50">
					<td colspan="4" width="100%" align="center" class="t">
						...
						<input type="image" name="enviar2" src="images/b_insert.png" alt="Agregar el llamado" onClick="return confirm('Estás seguro que queres agregar este llamado?');">
					</td>
				</tr>
			</form>
		<? }else{?>
...
<? }else{ // MODIFICAR?>
	<form method="post" name="modificar" action="<?=$_SERVER['PHP_SELF']?>">
		...
		$q = query("SELECT c.apellido, c.nombre, c.factura, c.localidad, c.cuit, c.dni, c.telefono, c.mail, ve.nombre AS agente, 
					ve.telefono AS tel_agente, ve.mail AS mail_agente, l.fecha, l.observaciones FROM llamados l
					INNER JOIN clientes c ON c.id_cliente = l.id_cliente
					INNER JOIN vendedores ve ON ve.id_vendedor = l.id_vendedor
					WHERE id_llamado = ". $_REQUEST['id_llamado']);
		...
		<table width="800" align="center" cellpadding="0" cellspacing="1" border="0">
			...
			<tr><td colspan="4"></td></tr>
			<tr height="30"><td class="t" colspan="4" align="center">Modificar el Llamado</td></tr>
			<tr height="40">
				<td class="c1">&nbsp;</td>
				<td class="c1" colspan="2">
					<table width="85%" border="0" cellspacing="0" cellpadding="0" align="center">
			...
			<tr height="50">
				<td colspan="4" width="100%" align="center" class="t">
					...
					<input type="hidden" name="id_llamado" value="<?= $_REQUEST['id_llamado']?>">
					...
					<input type="image" name="enviar2" src="images/b_insert.png" alt="Modificar el llamado" onClick="return confirm('Estás seguro que guardar estos cambios?');">
				</td>
			</tr>
		</table>		
	</form>
